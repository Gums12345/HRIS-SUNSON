...
<script type="text/javascript">
 ...
 var flag;
					$(document).ready(function() {
							...
							document.getElementById("jam").value = '<?php date_default_timezone_set('Asia/Jakarta'); echo date('H:i:s'); ?>';	
								
							autonumber('./urlautonumber?link=<?php 
							echo encrypt_url("SELECT CONCAT (". $this->session->userdata('admin_id')	.", DATE_FORMAT(CURDATE(),'%y'), DATE_FORMAT(CURDATE(),'%m')) AS idkaryawan, LPAD(IFNULL(MAX(RIGHT(nospl,3)), 0)+1 ,3, '00') AS anmbr FROM tspl WHERE SUBSTRING(nospl, -7, 2) = DATE_FORMAT(CURDATE(),'%y') AND SUBSTRING(nospl, -5, 2) = DATE_FORMAT(CURDATE(),'%m')") ;?>','nospl');
    ...
    <div class="box-header well" data-original-title="">
        <h2><i class="glyphicon glyphicon-user"></i> Tambah SPKL</h2>
 ...
 <!-- batas form spkl-->
  				<div class="box-header well" data-original-title="" style="margin-right:4px">
                        <h2><i class="glyphicon glyphicon-user"></i> Form SPKL </h2>
